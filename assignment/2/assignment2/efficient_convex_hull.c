...
t_line *upper_hull( t_point *points, int num_point, t_point p1, t_point pn, t_line *lines, int *num_line, int *capacity)
{
	if(num_point == 0) // 점이 없으면 p1pn이 upper hull
	{
		(*num_line)++;
		if((*num_line) >= (*capacity)) 
		{
			(*capacity) += 10; // 10씩 realloc
			lines = (t_line*)realloc(lines, sizeof(t_line) * (*capacity));
			fprintf(stderr, "realloc! (%d)\n", *capacity); // realloc 터미널 출력
		}

		int idx = (*num_line) - 1;
		lines[idx].from.x = p1.x;
		lines[idx].from.y = p1.y;
		lines[idx].to.x = pn.x;
		lines[idx].to.y = pn.y;
	...
	double a = pn.y - p1.y;
	double b = p1.x - pn.x;
	double c = p1.x * pn.y - p1.y * pn.x;

	for(int i = 0; i < num_point; i++)
	{
		float max_dist = distance(a, b, c, points[i]);
		if(dist < max_dist) // 거리가 최대인 점 = p_max
		{
			dist = max_dist;
			p_max = points[i];
		}
	...
	separate_points(points, num_point, p1, p_max, s1, s3, &n1, &n3); // s11과 정점 불가 구역.
	separate_points(points, num_point, p_max, pn, s2, s3, &n2, &n3); // s12와 정점 불가 구역

	lines = upper_hull(s1, n1, p1, p_max, lines, num_line, capacity); // s11에서 upper hull
	lines = upper_hull(s2, n2, p_max, pn, lines, num_line, capacity); // s12에서 upper hull
...
void separate_points( t_point *points, int num_point, t_point from, t_point to, t_point *s1, t_point *s2, int *n1, int *n2)
{
	...
	for(int i = 0; i < num_point; i++)
	{
		...
		if(a * X + b * Y - c < 0) // upper 
		{
			s1[(*n1)].x = X;
			s1[(*n1)].y = Y;
			(*n1)++;
		}
		else if(a * X + b * Y - c > 0) // lower
		{
			s2[(*n2)].x = X;
			s2[(*n2)].y = Y;
			(*n2)++;
